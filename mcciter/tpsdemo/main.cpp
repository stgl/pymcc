...
static void calc_tps()
{
  ...
  tpsdemo::Spline spline(control_points, regularization);
  ...
  for ( int x=-GRID_W/2; x<GRID_W/2; ++x )
  {
    ...
    {
      grid[x+GRID_W/2][z+GRID_H/2] = spline.interpolate_height(x, z);
    }
  }

  // Calc bending energy
  bending_energy = spline.compute_bending_energy();
}
